...
import constants.Constants.EModifyButtonItems;
...
public class CustomerModifyUILogic extends JDialog implements CustomerDialogUI {
	...
	public void initialize(CustomerUI ui, CustomerService service) {
		...
		for(ECustomerTableItems e : ECustomerTableItems.values()) {
			if(e.equals(ECustomerTableItems.ID)) {
				continue;
			}
		...
		for(EModifyButtonItems e : EModifyButtonItems.values()) {
		...
		this.setBounds(500, 500, 300, 230);
	...
	public class ButtonEvent implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			switch(EModifyButtonItems.valueOf(e.getActionCommand())) {
			case MODIFY:
				Customer customer = new Customer();
				
				//************customerUI.getSelected();
